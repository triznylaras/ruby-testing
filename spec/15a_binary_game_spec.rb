...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        wrong = '#'
        valid = '5'
        allow(game_input).to receive(:gets).and_return(wrong, valid)
      end
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        wrong2 = '%'
        valid = '7'
        allow(game_input).to receive(:gets).and_return(wrong1, wrong2, valid)
      end
  ...
  describe '#verify_input' do
    ...
    subject(:verify_game) { described_class.new(0, 10) }
  ...
  describe '#display_turn_order' do
    ...
    it 'increases guess_count by one' do
      expect { game_order.display_turn_order(binary_search_turn) }
        .to change { game_order.instance_variable_get(:@guess_count) }.by(1)
    end
